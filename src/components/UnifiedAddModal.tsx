    ...
    return (
        <Dialog open={isOpen} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px] w-[95%] max-h-[90vh] overflow-y-auto rounded-[2rem] pb-[env(safe-area-inset-bottom,1.5rem)]">
                <UnifiedAddForm
                    initialTab={prefillData?.type || initialTab}
...
function UnifiedAddForm({
    ...
    initialTab: 'expense' | 'income',
    prefillData?: UnifiedAddModalProps['prefillData'],
    onClose: () => void
}) {
    ...
    // Initialize state directly from props - no useEffect needed for reset
    const [name, setName] = useState(prefillData?.name || '')
    const [amount, setAmount] = useState(prefillData?.amount || '')
    const [expenseCategory, setExpenseCategory] = useState(prefillData?.type === 'expense' ? prefillData.category : 'Food')
    const [incomeCategory, setIncomeCategory] = useState(prefillData?.type === 'income' ? prefillData.category : 'Salary')
